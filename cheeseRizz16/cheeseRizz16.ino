...
void setup() {
  ...
  delay(2000);            // might need to remove for speedier bootup
  Serial.println("cheeseRizz16");
  //telemVal[0] = 15.3;  // bump version number display

  // Initialise CRSF for Arduino.
  crsf = new CRSFforArduino(&Serial1);  //A0=RX A1=TX on qtrp2040  D6,D7 = Serial1 on rp2350xiao  0,1=Serial1 on WSrp2350Zero
  if (!crsf->begin()) {
    crsf->end();
  ...
  //not availble on waveshare2350, only XIAO
  ...
  //maybe remove for speedier boot up
...
float calculateW() {  // probably takes more than 310us to get sensor data
  ...
  // // 1. Validate raw accelerations, if non number or inf then just use default minimum value
  // if (isnan(s.acceleration.x) || isinf(s.acceleration.x)) {
  //   if (DEBUG_HOTLOOP) Serial.printf(" Bad X-accel: %.2f\n", s.acceleration.x);
  //   return RPS_THRESHOLD * 2 * PI;  // Return safe value in radians/sec
  // }

  // if (isnan(s.acceleration.y) || isinf(s.acceleration.y)) {
  //   if (DEBUG_HOTLOOP) Serial.printf(" Bad Y-accel: %.2f\n", s.acceleration.y);
  //   return RPS_THRESHOLD * 2 * PI;  // Return safe value in radians/sec
  // }
